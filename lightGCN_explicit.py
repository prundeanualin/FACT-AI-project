...
import math
from user_models import LightGCN
...
checkpoint_dir = 'lightGCN_models_explicit'
...
save_model = True
only_load = True  # Set to False for enabling training loop, True for only loading model
lr = 0.001
...
lightGCN_model = LightGCN(n_users, n_movies, embedding_size=latent_embedding_size, n_layers=3,
                          min_rating=min_rating, max_rating=max_rating)

# Compute adjacency matrix
# Assuming your ratings DataFrame is named 'ratings'
# Replace this with the actual function call to compute_adjacency_matrix
adj_matrix = LightGCN.compute_adjacency_matrix(ratings, n_users, n_movies)
adj_matrix = torch.FloatTensor(adj_matrix.toarray())  # Convert to dense tensor

optimizer = torch.optim.Adam(lightGCN_model.parameters(), lr=lr, weight_decay=0.0000001)
n_batches = len(train_loader)

mse, rmse = lightGCN_model.evaluate(valid_loader=valid_loader, denormalize_predictions=True,
...
if not only_load:
    for epoch in range(1, epochs + 1):
        ...
        for batch_id, batch in enumerate(train_loader):
            ...
            # Include adjacency matrix in the forward pass
            predictions = lightGCN_model(user_indices, movie_indices, adj_matrix)
        ...
        if save_model and epoch % save_every == 0:
            checkpoint_path = f'{checkpoint_dir}/lightGCN_model_{feedback_type}_epoch_{epoch}.pth'
            lightGCN_model.save_model(checkpoint_path)
            print(f"{print_padding} Model saved at epoch {epoch} to {checkpoint_path} {print_padding}")
        print()

# Load the final model for prediction
lightGCN_model.load_model(f'{checkpoint_dir}/lightGCN_model_{feedback_type}_epoch_30.pth')
mse, rmse = lightGCN_model.evaluate(valid_loader=valid_loader, denormalize_predictions=True,
                                    denormalize_labels=True)
print(f"{print_padding} Validation MSE: {mse:.6f}, "
      f"Validation RMSE: {rmse:.6f} {print_padding}")
